...
let result = preserviceRows.filter(row => {
  ...
  if (statusFilter === 'Open') return status === 'ASSIGNED' || status === 'UNASSIGNED';
  if (statusFilter === 'Completed') return status === 'COMPLETED';
  return false;
});

  // 🔐 NEW: Filter by manager login
...
if (filterColumn && filterValue) {
  ...
  return result;
}, [preserviceRows, managerName, selectedGsp, filterColumn, filterValue]);
    ...
    <strong>
      {
        preserviceRows.filter(
          r =>
            (r['Manager'] || '').trim().toLowerCase() === managerNameRaw.trim().toLowerCase() &&
            (r['OWNER_HELPER'] || '').trim().toUpperCase() !== 'COMPLETED'
        ).length
      }
    </strong>
  </div>
...
<div style={{
  ...
  <button
    ...
    setPreserviceRows(updated);  // ✅ Triggers recomputation via useMemo
    setSelectedRows([]);         // ✅ Clears selection so table state is clean
    setCurrentPage(1);           // ✅ Optional: Reset to first page to avoid empty page
    }}
  ...
  {['All', 'Open', 'Completed'].map(status => (
    <button
      ...
      style={{
        padding: '6px 12px',
        borderRadius: '6px',
        border: statusFilter === status ? '2px solid #003b70' : '1px solid #ccc',
        backgroundColor: statusFilter === status ? '#003b70' : '#fff',
        color: statusFilter === status ? '#fff' : '#003b70',
        fontWeight: '500',
        cursor: 'pointer'
      }}
    >
